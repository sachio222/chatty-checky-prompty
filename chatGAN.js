// @/pages/api/chatGAN.js

// File: chatGAN.js
// Description: This file implements the Chatty, Checky, Prompty algorithm, which
// uses three OpenAI GPT models to collaboratively generate and validate prompts.
...
export default async function handler(req, res) {
  ...
  async function findCorrectPrompt(chattyPrompt, attempts) {
    ...
    // );
    validationResult = await checky.call(
      formatCheckyInput(chattyOutput.text, desiredOutput, true)
    );
    ...
    if (validationIsCorrect) {
      ...
      switch (action) {
        case 'suggest_prompt':
          promptyOutput = await prompty.call(
          ...
          // );
          // promptyOutput = await prompty.call(
          //   formatPromptyInput(chattyOutput, desiredOutput, promptHistory)
          // );
          console.log(
            `PROMPTY:\n[Attempt ${attempts + 1}]:`,
            promptyOutput.text
          );
          return findCorrectPrompt(promptyOutput.text, attempts + 1);

        case 'retry':
          // console.log(
          //   'RETRYING WITH PROMPTY',
          //   formatPromptyInput(chattyOutput.text, desiredOutput, promptHistory)
          // );
          ...
          console.log('PROMPTY', promptyOutput.text);
          // console.log('RETRYING', findCorrectPrompt(chattyPrompt, attempts + 1) )
          return findCorrectPrompt(promptyOutput.text, attempts + 1);
          return;
  ...
  const correctPrompt = await findCorrectPrompt(initialPrompt, 0);
...
Chatty cannot see the desired output. Chatty only has your prompt to help them.
